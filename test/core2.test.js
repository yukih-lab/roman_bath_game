...
function entryPointScenario() {
    ...
    let results = scenarios.reduce((acc, scenario, idx) => {
        ...
        while(!rbg.isNoSide(pA, pB)) {
            ...
            {
                try {
                    rbg.applyStrategy(pA, pB, scenario[turn * 2 - 2]);
                    result.score.push(getTurnHalfScore(turn, pA.name, pA, pB));
                } catch (e) {
            ...
            if (!rbg.isLoss(pB)) {
                try {
                    rbg.applyStrategy(pB, pA, scenario[turn * 2 - 1]);
                    result.score.push(getTurnHalfScore(turn, pB.name, pA, pB));
                } catch (e) {
                    result.message = e.message;
                    break;
                }
...
// スコアを取得
function getTurnHalfScore(turn, turnUser, p1, p2) {
    let per5 = (v) => v % 5;
...
function entryPointAutomatic() {
    ...
    while(!rbg.isNoSide(pA, pB)) {
        // 先攻
        {
            rbg.applyStrategyAutomatic(pA, pB, strategyIds[turn % 4]);
            outLog("after turn A", pA, pB);
        }

        // 後攻
        if (!rbg.isLoss(pB)) {
            rbg.applyStrategyAutomatic(pB, pA, strategyIds[turn % 4]);
